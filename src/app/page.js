...
export default function Home() {
  ...
  const [logs, setLogs] = useState([
    "System Ready. Waiting for input... (v1.0.2)",
  ]);
  ...
  const handleFileChange = (e) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      addLog(
        `Targets acquired: ${e.target.files.length} file(s). Select parameters.`,
      );
      setStatus("IDLE");
      setProgress(0);
      setElapsed(0);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/protect?mode=${mode}&format=${format}`,
        formData,
        {
      ...
      addLog(
        `Upload complete. Task ID: ${response.data.task_id.substring(0, 8)}...`,
      );
      addLog(
        `Engaging cloaking sequence (Level: ${mode.toUpperCase()}, Format: ${format.toUpperCase()})...`,
      );
      setProgress(20);
    } catch (err) {
      console.error(err);
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (taskStatus === "SUCCESS") {
            if (
              result &&
              (result.status === 1 || result.status === "SUCCESS")
            ) {
              setStatus("COMPLETE");
              const finalTime = result.elapsed_time || elapsed;
              addLog(`Cloaking Sequence Complete. Total Time: ${finalTime}s`);
              if (result.elapsed_time)
                setElapsed(Math.round(result.elapsed_time));
              setProgress(100);
              clearInterval(interval);
            } else {
              setStatus("ERROR");
              const errMsg =
                result && result.error ? result.error : "Unknown logic error";
              addLog(`ERR: ${errMsg}`);
              clearInterval(interval);
            }
    ...
    <div className="App">
      ...
      <div className="security-panel">
        ...
        <div className="panel-body">
          ...
          <div
            className={`drop-target ${selectedFiles ? "active" : ""}`}
            onClick={triggerFileSelect}
          >
            <div className="icon-large" style={{ marginBottom: "0.5rem" }}>
              {selectedFiles ? "📁" : "⌖"}
            </div>
            <div
              style={{
            ...
            >
              {selectedFiles
                ? `${selectedFiles.length} ASSETS SELECTED`
                : "DROP ASSETS [SCAN]"}
            </div>
            <input
              ref={fileInputRef}
